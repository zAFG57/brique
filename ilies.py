...
pas_casse = pygame.mixer.Sound('casse-brique/pas_casser.wav')
casse = pygame.mixer.Sound('casse-brique/brique.wav')
mur = pygame.mixer.Sound('casse-brique/touche.wav')
bruh = pygame.mixer.Sound('casse-brique/roblox.wav')
win = pygame.mixer.Sound('casse-brique/win.wav')
lose = pygame.mixer.Sound('casse-brique/lose.wav')



class Balle:
    def vitesse_plus(self):
        self.vx = (self.vx / self.vitesse) * (self.vitesse + 0.1)
        self.vy = (self.vy / self.vitesse) * (self.vitesse + 0.1)
        self.vitesse += 0.1
    ...
    def print_score(self):
        ascore = font.render(str(f"score: {self.score + (niveau*78)}"), 1, BLANC)
    ...
    def afficher(self):
        pygame.draw.rect(screen, BLEU_FONCER,(int(self.x-RAYON_BALLE), int(self.y-RAYON_BALLE),2*RAYON_BALLE, 2*RAYON_BALLE), 0)
...
class Raquette:
    ...
    def afficher(self):
        pygame.draw.rect(screen, BLEU_FONCER,(int(self.x-self.longueur/2),int(self.y-RAYON_BALLE),self.longueur, 2*RAYON_BALLE), 0)
...
class Jeu:
    def __init__(self):
        ...
        if niveau == 2:
            for y in range(13):
                self.brique.extend([Brique(((y*50)+100),70,4)])
                self.brique.extend([Brique(((y*50)+100),490,4)])
                self.brique.extend([Brique(100,(y*30+100),4)])
                self.brique.extend([Brique(700,(y*30+100),4)])
            for y in range(9):
                self.brique.extend([Brique(((y*50)+200),400,4)])
            self.brique.extend([Brique(300,210,4),Brique(500,210,4)])
        elif niveau == 1:
            for y in range(13):
...
while True:
    ...
    # game over 
        ...
        if jeu.instence == 0:
            ...
            jeu.instence += 1
        screen.fill(BLEU)
        fin = font.render(str("you win"), 1, BLANC)
        clicker = font.render(str("next level"), 1, BLANC)
        scor = font.render(str(f"your score: {jeu.balle.score + (niveau*78)}"), 1, BLANC)
        screen.blit(clicker, (250,height/2 + 50))
        screen.blit(fin, (250,height/2 - 50))
        screen.blit(scor, (250,height/2 - 150))
        for event in pygame.event.get():
            if event.type == pygame.QUIT: sys.exit()
            elif event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
